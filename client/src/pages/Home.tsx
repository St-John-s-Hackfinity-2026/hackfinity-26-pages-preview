      ...
      <section id="tracks" className="section tracks-section">
        ...
        <div className="track-grid">
          {tracks.map(([number, title, copy], index) => {
            ...
            return <Reveal key={title} delay={index * 0.06}><motion.article className={`track-card-flip ${isFlipped ? "is-flipped" : ""}`} aria-label={`${title} example projects`} onViewportEnter={() => setFlippedTrack(index)} onViewportLeave={() => setFlippedTrack(current => current === index ? null : current)} viewport={{ amount: .62, margin: "-18% 0px -18% 0px" }}><span className="track-card-inner"><span className="track-card-face track-card-front" aria-hidden={isFlipped}><span>{number}</span><ArrowDownRight /><h3>{title}</h3><p>{copy}</p><small>Approach the card to see example projects</small></span><span className="track-card-face track-card-back" aria-hidden={!isFlipped}><span>{number}</span><ArrowDownRight /><small>{exampleLabel}</small><strong>Example projects</strong><span className="track-example-list">{examples.map(example => <span key={example}>{example}</span>)}</span><small>Scroll away to return</small></span></span></motion.article></Reveal>;
          })}
        </div>
      </section>
